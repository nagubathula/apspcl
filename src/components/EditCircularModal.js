...
const EditModal = ({ isOpen, onClose, circular, onUpdate }) => {
  ...
  useEffect(() => {
    if (circular) {
      setFormData({
        ...
        startYear: circular.startYear || "",
        endYear: circular.endYear || "",
        circularNumber: circular.circularNumber || "",
        circularDate: circular.circularDate
          ? new Date(circular.circularDate).toISOString().split("T")[0]
          : "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `https://apspcl.ap.gov.in/api/circulars/${circular._id}`,
        {
          method: "PUT",
          headers: {
