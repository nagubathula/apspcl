...
const CIRCULARForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    formDataToSend.append("circularNumber", formData.circularNumber);
    formDataToSend.append("circularDate", formData.circularDate);
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage("Failed to create CIRCULAR. Please try again.");
      }
    } catch (error) {
      setMessage("An error occurred while creating the CIRCULAR.");
    } finally {
  ...
  return (
    <div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <div className="p-4">
          <h2 className="text-xl font-semibold mb-2">Create CIRCULAR</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {isSubmitting ? "Submitting..." : "Submit CIRCULAR"}
            </button>
          </form>
        </div>
      </Modal>
